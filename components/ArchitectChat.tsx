...
import { ChatMessage, ArchitectureNode, FileNode, KnowledgeDoc } from '../types';
...
const ArchitectChat: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const initChat = async () => {
    if (!chatSessionRef.current) {
        chatSessionRef.current = await gemini.createChatSession(state.projectData);
    }
  };

  useEffect(() => {
      if (!useLocalModel && !isLiveMode) initChat();
  }, [useLocalModel, isLiveMode]);
    ...
    try {
        ...
        if (useLocalModel) {
            if (!localEngine.isReady) {
                ...
                await localEngine.chatStream(messageToSend, (chunk) => {
                    fullText = chunk;
                    setMessages(prev => prev.map(m => m.id === aiMsgId ? { ...m, text: fullText } : m));
                });
            }
    ...
    <div className="flex flex-col h-full bg-[#0f172a] border-l border-glass-border w-96 flex-shrink-0 shadow-2xl relative z-40 transition-all duration-300">
      ...
      <div className="flex items-center justify-between p-4 border-b border-glass-border bg-slate-900/50">
        <div className="flex items-center gap-3">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white transition-colors ${useLocalModel ? 'bg-purple-600' : 'bg-brand-primary'}`}>
                {useLocalModel ? '🧠' : <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>}
            </div>
            <div>
                <h3 className="font-bold text-white text-sm">{useLocalModel ? 'Local Architect' : 'Cloud Architect'}</h3>
                <div className="flex items-center gap-1.5 cursor-pointer" onClick={() => setUseLocalModel(!useLocalModel)} title="Toggle Intelligence Provider">
                    ...
                    <span className="text-[10px] text-glass-text-secondary uppercase hover:text-white transition-colors">
                        {useLocalModel ? (localEngine.isReady ? 'Gemma-2B Active' : 'Model Offline') : 'Gemini Pro'}
                    </span>
                </div>
            </div>
        </div>
      ...
      <div ref={scrollRef} className="flex-grow overflow-y-auto p-4 space-y-4 custom-scrollbar bg-[#0b0e14]">
        {messages.length === 0 && (
            <div className="text-center p-6 text-glass-text-secondary text-sm">
                <p>👋 Hello! I'm your Thinking Architect.</p>
                <p className="mt-2">I have full context of your blueprints. Ask me to explain the stack, critique the security, or write custom code.</p>
            </div>
        )}
        {messages.map((msg) => (
        ...
        {isTyping && (
            <div className="flex items-center gap-2 text-glass-text-secondary text-xs p-2">
                <div className="w-1.5 h-1.5 bg-brand-secondary rounded-full animate-bounce"></div>
                <div className="w-1.5 h-1.5 bg-brand-secondary rounded-full animate-bounce delay-100"></div>
                <div className="w-1.5 h-1.5 bg-brand-secondary rounded-full animate-bounce delay-200"></div>
            </div>
        )}
      </div>
      ...
      <div className="p-4 bg-slate-900 border-t border-glass-border">
        <form onSubmit={(e) => handleSend(e)} className="relative">
            <input 
                ...
                placeholder={useLocalModel ? "Ask local model (Private)..." : "Ask cloud architect..."}
