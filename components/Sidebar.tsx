...
export default function Sidebar({ currentPhase, onPhaseClick, unlockedPhases }: SidebarProps) {
  const phases = Object.values(AppPhase);
  const currentIndex = phases.indexOf(currentPhase);

  return (
    <div className="hidden sm:flex w-20 md:w-24 border-r border-glass-border flex-col items-center py-6 gap-4 overflow-y-auto custom-scrollbar flex-shrink-0 bg-slate-900/30">
      {phases.map((phase, index) => {
        ...
        return (
          <button
            key={phase}
            onClick={() => !isDisabled && onPhaseClick?.(phase)}
            disabled={isDisabled}
            className={`
              group flex flex-col items-center gap-1.5 p-2 w-full transition-all duration-300 relative
              ${isDisabled ? 'opacity-30 cursor-not-allowed grayscale' : 'cursor-pointer hover:opacity-100'}
              ${isCurrent ? 'opacity-100' : 'opacity-60'}
            `}
          >
            {/* Indicator Bar for Active */}
            {isCurrent && (
              <div className="absolute left-0 top-1/2 -translate-y-1/2 h-8 w-1 bg-brand-accent rounded-r-full shadow-[0_0_10px_rgba(192,132,252,0.6)]" />
            )}
            ...
            <div className={`
              ...
              ${isCurrent 
                  ...
                  : isCompleted
                    ? 'bg-white/5 text-green-400 group-hover:bg-white/10'
                    : 'bg-transparent text-slate-400 group-hover:bg-white/5 group-hover:text-slate-200'
              }
            `}>
            ...
            <span className={`
              text-[10px] font-bold tracking-wide uppercase transition-colors text-center leading-tight
