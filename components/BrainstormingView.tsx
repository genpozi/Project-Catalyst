...
import { BrainstormingData, Persona } from '../types';
...
const BrainstormingView: React.FC<BrainstormingViewProps> = ({ data, onUpdate, onRefine, isRefining = false, onContinue }) => {
  ...
  const handlePersonaChange = (index: number, field: keyof Persona, value: string) => {
    ...
    if (field === 'painPoints') {
        // Handle array conversion if needed, but for now we treat painPoints as editable array elsewhere?
        // Actually for simplicity, let's keep pain points as strings in the UI for edit? 
        // No, let's just update specific pain point or add/remove. 
        // For this function, assuming value is the string for a specific field.
        // Special case below.
    } else {
      ...
      {onRefine && (
        <div className="max-w-3xl mx-auto mb-8">
            <RefineBar 
                ...
                placeholder="e.g. 'Add a persona for an Administrator', 'Make the features more focused on mobile'" 
            />
        </div>
      )}
      
      <div className="space-y-8">
        {/* Top Row: USPs and Questions */}
                ...
                {data.features.map((feature, idx) => (
                    <div key={idx} className="relative group">
                        <input 
                            ...
                            className="px-3 py-1.5 bg-slate-800 text-blue-200 rounded-full border border-slate-600 text-sm focus:border-brand-secondary focus:outline-none w-48 sm:w-auto"
                        />
                        <button 
